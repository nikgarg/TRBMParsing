...
# Adapted to 2009 format (mm) <
    ...
    @fields_g{'word', 'pos', 'head', 'dep'} = (split (/\s+/, trim($line_g)))[1, 5, 8, 10] ; #nikhil - may be change 5 to 4
    # Using ppos instead of pos to be consistent with 2008 st where pposs was used

    push @{$sent_gold}, { %fields_g } ;

    @fields_s{'word', 'pos', 'head', 'dep'} = (split (/\s+/, trim($line_s)))[1, 5, 8, 10] ; #nikhil - may be change 5 to 4   
# Adapted to 2009 format (mm) >
